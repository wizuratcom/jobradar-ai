...
def raw_from_mapping(payload: Mapping[str, object], *, source_name: str, source_url: str | None = None) -> RawJobData:
    ...
    return RawJobData(
        ...
        raw_stack_skills=[_string(item) or "" for item in stack] if isinstance(stack, list) else None,
...
def extract_text(text: str, *, source_name: str = "text") -> RawJobData:
    ...
    if mode is None:
        ...
        if has_hybrid or (has_remote and has_onsite):
            ...
            mode = "onsite"
    required_line = next((line.split(":", 1)[1] for line in lines if line.casefold().startswith(("required skills:", "required:", "requirements:", "обязательные требования:"))), "")
    preferred_line = next((line.split(":", 1)[1] for line in lines if line.casefold().startswith(("preferred:", "будет плюсом:"))), "")
    stack_line = next((line.split(":", 1)[1] for line in lines if line.casefold().startswith(("stack:", "tech stack:"))), "")
...
def _required_experience_from_text(text: str) -> tuple[str | None, int | None, str | None]:
    ...
    if russian:
        return clean_text(russian.group(0)), int(russian.group("years")), clean_text(russian.group("area"))
    match = re.search(
    ...
    return value, int(years.group(1)) if years else None, clean_text(area.group(1)) if area else None
