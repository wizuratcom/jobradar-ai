...
class AssessmentRepository:
    ...
    async def create(
        self, *, job_id: int, user_id: int, job_match_id: int | None, provider: str,
        model: str | None, grade: int, purpose: str, analysis: AssessmentResult,
        prompt_version: str, input_tokens: int | None = None,
        cached_input_tokens: int | None = None, output_tokens: int | None = None,
        reasoning_tokens: int | None = None,
    ) -> JobAnalysis:
        record = JobAnalysis(
            job_id=job_id, user_id=user_id, job_match_id=job_match_id,
            provider=provider, model=model, recommendation=analysis.verdict,
            analysis_payload=analysis.model_dump(mode="json"), grade=grade,
            prompt_version=prompt_version, fit_score=analysis.fit_score,
    ...
    async def list_for_user(self, user_id: int, limit: int, offset: int, job_id: int | None = None):
        ...
        records = list((await self.session.scalars(query.order_by(JobAnalysis.id.desc()).limit(limit).offset(offset))).all())
